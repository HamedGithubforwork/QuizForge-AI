...
test('diagnostics discard secrets and unknown provider error text',()=>{
  ...
  diagnostic.tokenResponse(400); diagnostic.tokenFailure(); diagnostic.scriptFailure(); diagnostic.scriptResponse(504)
  ...
  assert.equal(result.script_error,true)
  assert.equal(result.script_http_failure,504)
