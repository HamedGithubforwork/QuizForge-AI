...
class FrontendHotfixTests(unittest.TestCase):
    def test_remote_hotfix_is_frontend_only_with_atomic_rollback(self):
        ...
        self.assertIn("Continue to your existing account", REMOTE_APPLY)
        self.assertIn("New here? Start with a fresh account", REMOTE_APPLY)
        self.assertNotIn('b"Sign in or create account"', REMOTE_APPLY)
        self.assertIn("quizfromnotes.com:443:127.0.0.1", REMOTE_APPLY)
        self.assertIn("https_ready=0", REMOTE_APPLY)
        self.assertIn("for _ in $(seq 1 30)", REMOTE_APPLY)
        self.assertIn('test "$https_ready" -eq 1', REMOTE_APPLY)
