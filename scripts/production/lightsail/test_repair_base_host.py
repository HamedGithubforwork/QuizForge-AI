...
class BaseHostRepairTests(unittest.TestCase):
    def test_remote_repair_is_base_host_only(self):
        ...
        self.assertIn('application_state="clean"', REMOTE_REPAIR)
        self.assertIn('application_state="staged"', REMOTE_REPAIR)
        self.assertIn("test -L /opt/quizforge/current", REMOTE_REPAIR)
        self.assertIn("test -s /opt/quizforge/frontend/index.html", REMOTE_REPAIR)
        self.assertIn("! systemctl is-active --quiet quizforge.service", REMOTE_REPAIR)
        self.assertIn("! systemctl is-enabled --quiet quizforge.service", REMOTE_REPAIR)
