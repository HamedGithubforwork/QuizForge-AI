...
class PublicLaunchDiagnosticTests(unittest.TestCase):
    def test_diagnostic_is_read_only_and_bounded(self):
        ...
        self.assertIn('combined("docker","logs","--tail","200"', REMOTE_DIAG)
        self.assertIn("connectionrefusederror", REMOTE_DIAG.lower())
        self.assertIn("log_signals", REMOTE_DIAG)
        self.assertIn("health_signals", REMOTE_DIAG)
        self.assertIn("configured_services", REMOTE_DIAG)
        self.assertIn('combined("sudo","-n","ss","-H","-ltnp")', REMOTE_DIAG)
        self.assertIn('"listeners"', REMOTE_DIAG)
        self.assertIn('combined("sudo","-n","journalctl","-u","quizforge.service"', REMOTE_DIAG)
        self.assertIn("journal_signals", REMOTE_DIAG)
        self.assertIn("dependency_unhealthy", REMOTE_DIAG)
