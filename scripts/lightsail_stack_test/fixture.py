"""Disposable stack capacity fixture derived from reviewed runtime rehearsal.

Never connects to AWS or uses live identities/model keys. Explicit host marker,
fresh paths and a bounded cgroup are required outside ephemeral GitHub CI.
"""
import json
import os
...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / 'candidate/scripts/production'))
...
def main():
    ci = os.environ.get('GITHUB_ACTIONS') == 'true' and bool(os.environ.get('RUNNER_TEMP'))
    if not ci and not Path('/var/lib/quizforge-capacity-ready').is_file():
        raise ValueError('Disposable capacity-host marker required')
    if any(Path(p).exists() for p in ('/etc/quizforge', '/var/lib/quizforge', '/opt/quizforge/frontend')):
        raise ValueError('Refusing to touch existing runtime paths')
    root = Path(os.environ['RUNNER_TEMP'] if ci else '/home/ubuntu') / 'lightsail-runtime'
    run('cp', str(Path(__file__).resolve().parents[2] / 'candidate/scripts/production/lightsail/quizforge.slice'), '/etc/systemd/system/quizforge.slice')
    run('systemctl', 'daemon-reload')
    run('systemctl', 'start', 'quizforge.slice')
    render(fixture(), root)
...
'''
        ...
        exercise(root, command, env, ci=ci)
