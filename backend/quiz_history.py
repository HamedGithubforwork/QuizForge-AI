...
async def get_history_repository(
    ...
    if backend == "postgres":
        ...
        if pool is None:
            raise HTTPException(503, "Quiz history is temporarily unavailable.")
        # The issuer is server configuration used by get_current_user, never request data.
        return PostgresHistoryRepository(pool, issuer=app_shared.SUPABASE_URL + "/auth/v1",
