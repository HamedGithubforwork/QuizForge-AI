...
test('rejects malformed phone numbers before network access', async () => {
  ...
  await assert.rejects(
    () => beginPhoneVerification('access-token', '613-555-1234'),
    /country code/i,
  )
...
test('verifies six-digit phone code', async () => {
  ...
  await verifyPhoneNumber('access-token', '123456')
  assert.deepEqual(body, {
    AccessToken: 'access-token',
    AttributeName: 'phone_number',
    Code: '123456',
...
test('switches preference while keeping both factors enabled', async () => {
  ...
  await setMfaPreference('access-token', 'sms')
  await setMfaPreference('access-token', 'totp')
...
test('provider verification errors are sanitized', async () => {
  ...
  await assert.rejects(
    () => verifyPhoneNumber('access-token', '123456'),
    /not accepted/i,
  )
})

test('phone display masks all but the last four digits', () => {
  assert.equal(maskedPhoneNumber('+16135551234'), '••••1234')
